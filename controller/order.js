        ...
        if (cart) {
          ...
          if (req.body.payment == "COD") {
            const newOrder = new Order({
              ...
              billamount: totalCost - (req.body.couponAmount ? req.body.couponAmount : 0),
      ...
      if (expiryDate >= currentDate) {
        ...
        if (req.body.cartAmount > couponData.minimum_purchase) {
          ...
          if (couponData.claimed_users.length <= 0) {
            ...
            res.json({ couponAvailable: true, couponAmount, couponName, cartAmount: req.body.cartAmount })
          } else {

            let isClaimed = couponData.claimed_users.findIndex((element) => element.toString() === userId)

            if (isClaimed === -1) {
              let couponAmount = couponData.discountAmount
              res.json({ couponAvailable: true, couponAmount, cartAmount: req.body.cartAmount })
            } else {
