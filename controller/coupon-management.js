...
const updatecoupon = (req, res) => {
    Coupon.findOneandUpdate({ code }, { discount }, { user_allowed }, { minimum_purchase }, { expiry }, (error) => {
        if (error) {
            res.send('Something went wrong')
        } else {
...
const listcoupon = async (req, res) => {
    try {
        ...
        await Coupon.findOneAndUpdate({ _id: id }, { $set: { status: 'Listed' } })
            .then(() => {
                res.redirect('/admin/coupontable')
            })
    } catch (err) {
...
const unlistcoupon = async (req, res) => {
    try {
        const id = req.params.id
        await Coupon.findOneAndUpdate({ _id: id }, { $set: { status: 'Unlisted' } })
            .then(() => {
                res.redirect('/admin/coupontable')
            })
    } catch (err) {
