...
const category = (req,res)=>
{
    ...
    res.render('user/category',{categorylist:req.categorylist,productlist:req.productlist})
...
const loadEdituser=async(req,res)=>{
    try {
       const data = await User.findOne({_id:req.user._id})
          res.render('user/edituser',{address:data.address,username:data.username,email:data.email})
...
const editAddress = async(req,res)=>{
    try {
        await User.updateOne({_id:req.user._id,'address._id':req.params.id},{
            $set:{
                address:{
                    name:req.body.name,
...
const cancelOrder = async(req,res)=>{
    try {
        ...
        const order= await Order.findOneAndUpdate({_id:id},{$set:{status:"Canceled"}},{new:true})
        stockinc(order)
...
function stockinc(data) {
    data.products.forEach(async product=>{
        const item = await Product.updateOne(
          {_id:product.product},{$inc:{stockQuantity:1}},{new:true}
        )
